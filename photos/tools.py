...
def handle_photo():
   ...
   # 最后将data.json文件存到博客的source/photos文件夹下
	...
	for index,row in df.sort_values(axis = 0,ascending = False,by ='date').iterrows():
		...
		if i == 0:  # 处理第一个文件
			new_dict = {"date": year_month, "arr":{'year': date.year, 'month': date.month, 'size': [imgsize], 'link': [filename], 'text': [date.strftime('%m月%d日%H点%M分' )], 'type': ['image'] } } 
			list_info.append(new_dict)
		elif year_month != list_info[-1]['date']:  # 不是最后的一个日期，就新建一个dict
			new_dict = {"date": year_month, "arr":{'year': date.year, 'month': date.month, 'size': [imgsize], 'link': [filename], 'text': [date.strftime('%m月%d日%H点%M分' )], 'type': ['image'] } }
			list_info.append(new_dict)
		else:  # 同一个日期
			list_info[-1]['arr']['size'].append(imgsize)
			list_info[-1]['arr']['link'].append(filename)
			list_info[-1]['arr']['text'].append(date.strftime('%m月%d日%H点%M分' ))
